<?php
    ...
    function nv_my_create_webhook($event, $version = '2.0'){
        ...
        if ($version == '2.0') {
            ...
            if ($event == 'Picked Up') {
                // Accepts `Picked Up, In Transit To Origin Hub`
                return false;
            }
            $data = [
                'event' => $event,
    ...
    function nv_my_sync_webhooks($is_native = false){
        ...
        if ($version == '2.0') {
            // Remove `Picked Up` and add `Picked Up, In Transit To Origin Hub`
            $event_statuses = array_diff($event_statuses, ['Picked Up']);
            $event_statuses = array_merge($event_statuses, ['Picked Up, In Transit To Origin Hub']);
