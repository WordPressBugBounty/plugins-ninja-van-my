...
class WooCommerce
{
    ...
    public function add_ninja_van_column_contents($column, $post_id) {
        if ('ninja' === $column) {
            ...
            echo '<div class="ninja_van_order_column">';
            ...
            if ($tracking_number) {
                ...
                if ($order->get_meta('_ninja_van_cancelled_payload', true) && isset($log) && $log != 'Order was cancelled') {
                    echo '<div class="column_message column_error"><b>'.esc_html__('Cancel Error: ', 'ninja-van-my').'</b>';
                    echo '<div class="column_content">'.esc_html($log).'</div></div>';
                } 
                ...
                if ($order->get_meta('_ninja_van_payload', true) && isset($log) && $log != 'Successfully Pushed To Ninja Van!') {
                    echo '<div class="column_message column_error"><b>'.esc_html__('Booking Error: ', 'ninja-van-my').'</b>';
                    echo '<div class="column_content">'.esc_html($log).'</div></div>';
                } else {
                    echo 'Not Available';
                }
